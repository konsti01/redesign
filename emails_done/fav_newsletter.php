...
<body style="margin: 0px; padding: 0px; font-family: Arial, Helvetica, sans-serif; color: #555555; font-size: 1em; margin-top: 0.5%;">
    ...
    <div class="wrapper" style="background-color: #ffffff; border: 1px solid #dddddd; border-radius: 0.5vw; width: 99%; margin: 0 auto 0 auto;">
        ...
        <div class="content">
            <h1>
                <a target="_blank" href="https://www.meska.hu/MyFavouriteSellersProducts?code={{.User.Code}}">Kedvenc alkotóid legújabb termékei a Meskán</a>
            </h1>
            {{ $s := .}}
            {{ range .Products }}
            <div>
                <a href="https://www.meska.hu/Link?id={{$s.SendID}}&pid={{.ID}}"><img src="https://www.meska.hu/img/product/thumbnail/{{.ImageURL}}"/></a>
                <a href="https://www.meska.hu/Link?id={{$s.SendID}}&pid={{.ID}}">{{.ShopName }} - {{ .ProductName }} - {{.Price}} Ft</a>
            </div>
            {{ end }}
        </div>
        <!-- CONTENT END -->
