...
A mai napon <?=App_Helper_Domain::hungarize($domain, true, 'n')?> megvásárlásra kerültek az alábbi termékeid:<br><br>
	...
	<table style="<?=$table_style?>">
		<tr>
			<td style="<?=$td_style?> width: 350px; text-align: left; <?= App_Helper_PostagePrice::is_address_needed_by_transfer_mode($_SESSION['transfer_mode_' . $shop_name]) ? ' border-right: 1px solid #' . $color . ';' : '' ?>">
				...
				Felhasználónév: <a href="<?=App_Controller::$_protocolStatic?>://<?=$_user->user_name?>.alkotokboltja.hu" style="<?=$a_style?>"><?=$_user->user_name?></a><br>
				Név: <?=$_user->name?><br>
