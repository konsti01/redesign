		...
		<table style="<?=$table_style?>">
			<tr>
				<td rowspan="2" style="<?=$td_style?> width: 110px;">
					<a href="<?=App_Controller::$_protocolStatic?>://www.izporta.hu/ProductView/index/<?=$product['id']?>" style="display: block; margin:0 16px 0 0; padding:5px; border:1px solid #d4d9d1; height: 80px; width: 80px;">
				...
				<td colspan="2" style="<?=$td_style?>">
					<a style="<?=$a_style?> font-weight: bold;" href="<?=App_Controller::$_protocolStatic?>://www.izporta.hu/ProductView/index/<?=$product['id']?>">
	...
	<table style="<?=$table_style?>">
		<tr>
			<td style="<?=$td_style?> width: 350px; text-align: left; <?= App_Helper_PostagePrice::is_address_needed_by_transfer_mode($_SESSION['transfer_mode_' . $shop_name]) ? ' border-right: 1px solid #' . $color . ';' : '' ?>">
				...
				Felhasználónév: <a href="<?=App_Controller::$_protocolStatic?>://<?=$_user->user_name?>.izporta.hu" style="<?=$a_style?>"><?=$_user->user_name?></a><br>
				Név: <?=$_user->name?><br>
